#include "cppNetworkUtilImpl.h" // 这里包含实现类的定义
...
{
    pimpl_->sendDataToHttpHost_impl(host, path, port, header, content);
}
...
{
    pimpl_->sendDataToHttpsHost_impl(host, path, port, header, content, enable_CA);
}

cppNetworkUtil::cppNetworkUtil() : pimpl_(std::make_unique<cppNetworkUtilImpl>()) {}

cppNetworkUtil::~cppNetworkUtil() = default; // unique_ptr 会自动管理内存

void cppNetworkUtil::printOpensslVersion()
{
    pimpl_->printOpensslVersion_impl();
}

void cppNetworkUtil::run(int port, serverCallback *callback)
{
    pimpl_->run_impl(port, callback);
}